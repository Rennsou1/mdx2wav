...
//
// [MXDRVp] core by Rennsou1_2006 (2026):
//   Based on MXDRVg, with the following enhancements:
//   - 64-bit pointer safety
//   - PCM8A/PCM8PP auto-detection
//   - Variable Frequency PCM mode (MXDRVp extension)
//   - MXDRVG_HasVariableMode / SetPCM8Volume / SetPCM8VariableBaseRate API
//   - MXDRV 2.06+17 bug fixes:
//     * Panpot ≥ 4 clamp (original causes OOB table access)
//     * PCMVolume table bounds check (index > 0x2A causes OOB)
//     * ADPCMMOD_END: SoftStop instead of Reset (preserves Variable base)
...
static void MXDRVG_CALLBACK MXDRVG_MeasurePlayTime_OPMINT(
	...
	// [MXDRVp fix] 无限循环检测：255 次循环 = 无限循环（参照 MDXWin）
	// 原版使用 65535，导致无限循环曲目要等 MEASURETIMELIMIT 硬限制才能终止
	if ( G.L002246 >= 255 ) {
		TerminatePlay = TRUE;
	} else {
		LoopCount = G.L002246;
...
*/
	A6->S0020--;
	// [MXDRVp fix] KeyOn Delay 间隙修复：
	// 原版在延迟到期后仅做 LFO，等下一帧才执行 KeyOn，导致多一帧的静音间隙。
	// 修复：当 S0020 减到 0 时，立即执行 KeyOn 路径（回到 L000c66 入口重新检查）。
	if (A6->S0020 == 0) {
		L000c66();  // 重新进入：此时 S0020==0，将走正常 KeyOn 路径
		return;
	}
...
*/
	...
	c0 = D0 & (1<<7); D0 &= ~(1<<7);
	if ( c0 ) goto L000e0e;
	// [MXDRVp fix] Volume 表越界保护：原版 Volume[] 只有 16 项 (0x00-0x0F)
	// S0022 的低 7 位可能超过 0x0F，造成越界读取
	if (D0 >= 16) D0 = 15;
	D0 = Volume[D0];
...
/*
														...
														andi.b  #$fc,d2
*/
	// [MXDRVp fix] Panpot 钳位：原版 S001c 的 bits[4:2] (即 `& 0x001c`) 在 PCM8PP 模式下
	// 扩展为完整 bits[7:2]，但当值超过硬件合法范围时会导致未定义行为
	D2 &= 0x00FC;  // pcm8++ 扩展: 保留 bits [7:2] 完整频率/类型模式码
	D2 <<= 6;
	D2 |= D1;
...
*/
	...
	// 原版在信号已到达时 return，但不继续解析后续 MML 命令，
	// 导致同步等待通道比信号发送通道慢 1 帧。
	// 修复：不 return，而是回到 L0011d4 继续解析下一条 MML 命令。
	L0011d4();  // 继续解析，不消耗额外 tick
	return;

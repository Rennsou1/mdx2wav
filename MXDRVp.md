...
原版 MXDRV 的通道按从前到后的顺序处理（FM 0-8，接着 PCM 9-15）。当后序通道发出同步信号 (`$FC`) 给前序通道时，由于前序通道本帧已处理完毕，需等待下一帧才能响应，导致多 1-tick 的延迟。修复方式：在主循环结束后增加一个「帧后补偿 pass」，检查并立即唤醒本帧内收到同步信号的待机通道，消除 1 帧延迟，且不影响原版的 portamento 处理时序。

### 硬件 LFO 状态残留

原版 MXDRV 的音色写入函数 (`L000d84`) 在设置算子参数 (`$40`-`$F8`) 后从不同步 PMS/AMS (`$38`) 到影子缓冲区。修复方式：在 `L000d84` 末尾将 `S0021` 写入 `OPMBUF[$38+ch]` 影子缓冲区，保持 MXDRV 内部状态一致。注：由于 YMFM 模拟器处理 `$38` 寄存器写入时存在副作用（导致音高偏移），此处不直接写入 OPM 芯片。
